...
The package assumes that there is a sub directory "LocalCopyDWD/" in your R project and defines strings to the copies of recent and historical DWD data `r LocalCopy_DWD_ftp_historical`.
...
```{r getDWDContent, eval=TRUE, message=FALSE, warning=FALSE, include=TRUE}
...
- "stations" a data frame with the station meta data
- "ziplist" a list of the ZIP files for the stations
- "zipID" a list of the station IDs

### Meta data for additional precipitaiton stations

For the additional data from stations measuring precipitation data the function "getDWDContent" has to be called with the respective parameter "precipitation = TRUE".

```{r getDWDContent_precipitation, eval=TRUE, message=FALSE, warning=FALSE, include=FALSE}
DWDRain_content <- getDWDRainStationList(recent = DWDRain_ftp_recent, historical = DWDRain_ftp_historical)

outputDir <- here(Local_R_DWD) 
if (!dir.exists(here(outputDir))) {dir.create(here(outputDir))}
fn <- paste0(here(outputDir), fn_DWDRain_content)
...
The DWD records for historical observations consists of data different stations (which changed over time) and for the recent observations of  stations.
...
```{r getDWDStationList, message=FALSE, warning=FALSE}
...
For the file transfer the package holds the function "Copy_DWD_ZipFiles" for downloading the files to the local directory, which has do be exist on the local machine.
...
if (!dir.exists(here(LocalCopy_DWD_ftp_recent)) 
    {dir.create(here(LocalCopy_DWD_ftp_recent))}
...
```{r getsingleDWDWeather, message=FALSE, warning=FALSE, echo=F}
...
recent_data <- getsingleDWDWeather(station = selstation, ziplist = DWD_content$recent$ziplist,
                            repository = DWD_ftp_recent, local = F, quiet = F)
```

```{r getsingleDWDWeatheroutput, message=FALSE, warning=FALSE}
head(recent_data)

```
In addition to the data from produkt_klima_tag_YYYYMMDD_YYYYMMDD_nnnnn.txt, the measurement height of wind speed was retrieved from the metadata file of the selected weatherstations and a Date column is added in R format.
...
combined_data <- rename.weather(combined_data)
str(combined_data)
...
```{r}
test <- NULL
fn <- "weather_dat_1990.fst"
path <- paste0(here(Local_R_DWD, fn))

weather_historic <- read.fst(path = path)
```
...
The function InterpolateFromDWD is used to interpolate the weather data for a selected location. The procedure is splitted in two parts, first the interpolation of the historical data and second the interpolation of the recent data, finally the data is combined in one data frame.
...
```{r LoadDataForInterpolation, message=FALSE, warning=FALSE, eval=TRUE}
...
fn_recentDWD_data <- paste0("recent_weather_dat_", as.character(ThisYear-1),".fst")
...
```{r InterpolateHistoricData, message=FALSE, warning=FALSE, eval=TRUE}
...
### interpolate the historic weather data for the location  
  ...
  RainStations_selected <- SelectStations (lat=geoBreite,
                                          long=geoLaenge,
                                          height_loc=Hoehe_m, 
                                          stationlist = DWDRain_content$historical$stationlist,
  ...
  DWDdataHistorical <- InterpolateFromDWD(df_DWD_core = weather_historic_core, stationlist = stationlist, 
  ...
  DWDdataHistorical <- DWDdataHistorical %>% filter(Time < ExcelFirstDayofYear)
  
  # Add longitude and latitude to the data frame
  DWDdataHistorical$Longitude <- geoLaenge
  DWDdataHistorical$Latitude <- geoBreite
  summary(DWDdataHistorical)
...
```{r InterpolateRecentData, message=FALSE, warning=FALSE, eval=TRUE}
...
# recent data #######
  ...
  #
  DWDdatarecent <- InterpolateFromDWD(df_DWD_core = weather_recent, stationlist = stationlist, geoBreite = geoBreite, geoLaenge = geoLaenge, 
                                max.Height.Distance_m=100, Hoehe_m = Hoehe_m, df_Rain = df_Rain, 
                                startdate = startdate) 
  
  DWDdatarecent$Longitude <- geoLaenge
  DWDdatarecent$Latitude <- geoBreite
  
  summary(DWDdatarecent)
...
```{r}
  ...
  DWDdata <- rbind(DWDdataHistorical, DWDdatarecent)
  summary(DWDdata)
