...
## Critical issues
- **Package load side effects.** `WeatherFunctions.R` creates directories under `tools::R_user_dir()` during package load via top-level code, which can cause CRAN checks to fail because packages should not write to the file system at load time. Move directory creation inside functions or an explicit setup step instead of executing at the top level. 【F:R/WeatherFunctions.R†L31-L118】
