...
#  path <- file.path(path, subDir)
  ...
  for (i in 1:length(stationlist$Stations_id)){
    ...
    # get the name of the unzipped file with the weather data
      fn_Meta <- here(path_unzip, paste0(fn_Metadaten, id, ".txt"))
...
#' @export
#'
#' @examples Copy_DWD_ZipFiles("ftp://ftp-cdc.dwd.de/pub/CDC/observations_germany/climate/daily/kl/recent/",
...
UpdateDWDData_to_fst <- function(dataperiod="recent", startdate="1990-01-01", isloadnew=T,
  ...
  if (MinDataset==TRUE) {
    fn <- paste0("weather_dat_min_",as.character(startyear),".fst")
  }

  # save data to fst file
  if (dataperiod == "recent") {
    write.fst(x = df, path = paste0(Local_R_DWD, "/recent_", fn))}
  if (dataperiod == "historical") {
    write.fst(x = df, path = paste0(Local_R_DWD, "/historical", fn))}
}
...
getsingleDWDRain <- function(station, ziplist, repository, local=F, quiet=T){
  ...
  # filename <- paste0(path, "/", filename)
  dest_file <- here("DWD_tmp", filename)
  ...
  if(file.exists(here("DWD_tmp","unzip"))){
    unlink(here("DWD_tmp","unzip"), recursive = TRUE, force = TRUE)
  }

  if (local){
    if (file.exists(paste0(repository, "/",filename))){
      unzip(paste0(repository, "/",filename), exdir = here("DWD_tmp","unzip"))
      filename <- grep("produkt_nieder_tag_", list.files(here("DWD_tmp","unzip/")), value = TRUE)}
    else {
      return <- NULL
      stop()
    }
  } else {
    unzip(here("DWD_tmp", filename), exdir = here("DWD_tmp","unzip"), overwrite = T)
    filename <- grep("produkt_nieder_tag_", list.files(here("DWD_tmp","unzip")), value = TRUE)
  }
  ...
  DWDWeather <- read.table(file = here("DWD_tmp", "unzip", filename),
                           header=TRUE, sep=";", quote="", dec=".", na.strings=c("-999", "\032"),
                           stringsAsFactors=FALSE, strip.white=TRUE, fill=TRUE, fileEncoding="latin1")
  DWDWeather$Date <- as.Date(as.character(DWDWeather$MESS_DATUM), "%Y%m%d")
...
#' @export
#'
#' @examples DWDisMeasured(weather_historic)
...
# use rain data from nearest station if available
  ...
  InterpolateFromDWD <- function(df_DWD_core, stationlist, geoBreite, geoLaenge,max.Height.Distance_m=100, Hoehe_m = 100, df_Rain=NULL,
    ...
    df_DWD_core <- merge(x = df_DWD_core,
                         y = stationlist[,c("Stations_id", "Distance_km","Stationshoehe")], by = "Stations_id", all.x = T)
    ...
    long_data <- melt(df_DWD_core, id.vars = id_vars, measure.vars = longNames_DWD_core)

    if (!is.null(df_Rain) ){
      df_Rain$value <- df_Rain$RS
    ...
    # select the first three values for each date and variable which are not NA
    ...
    xport$geoBreite <- geoBreite$Latitude
    xport$geoLaenge <- geoLaenge$Longitude
